...
class TestSyncEngine:
    ...
    def test_initial_sync_and_diff(self, app, session, default_user):
        sync_user_groups(default_user, "keycloak", _refs("keycloak", "/a", "/b"))
        first = _memberships(session, default_user)
        assert set(first) == {"/a", "/b"}

        sync_user_groups(default_user, "keycloak", _refs("keycloak", "/b", "/c"))
    ...
    def test_group_upsert_updates_display_name(self, app, session, default_user):
        sync_user_groups(default_user, "keycloak", _refs("keycloak", "/a"))
        sync_user_groups(
            default_user,
            "keycloak",
    ...
    def test_fail_closed_on_malformed_claim(self, app, session, default_user):
...
class TestKeycloakBackend:
    ...
    def test_extract_memberships(self, backend):
        with patch.object(
            ...
            "_get_group_by_path",
            side_effect=(
                {"id": "atlas-id", "name": "atlas", "path": "/local/atlas"},
                {
                    "id": "readers-id",
                    "name": "readers",
    ...
    def test_search_groups_flattens_subgroups(self, backend):
        ...
        assert [r.external_id for r in refs] == ["local-id", "atlas-id"]

    def test_search_groups_respects_limit(self, backend):
        tree = [
            {"id": f"id-{i}", "name": f"g{i}", "path": f"/g{i}", "subGroups": []}
            for i in range(30)
        ]
        with patch.object(backend, "_admin_get", return_value=tree):
    ...
    def test_group_exists(self, backend):
        with patch.object(backend, "_admin_get", return_value={"id": "abc"}):
    ...
    def test_fetch_memberships_paginates(self, backend):
        ...
        page_one = [
            {"id": f"id-{i}", "name": f"g{i}", "path": f"/g{i}"} for i in range(100)
        ]
        page_two = [{"id": "last-id", "name": "last", "path": "/last"}]
        with patch.object(

...
def naive_utcnow() -> datetime:
    ...
    REANA stores every timestamp in naive UTC columns (``sqlalchemy_utils``'s
    ``Timestamp`` mixin uses plain ``DateTime``), so values compared against
    those columns must be naive too. This computes it from a timezone-aware
    clock and drops the offset, avoiding ``datetime.utcnow()`` which is
...
_CSV_FORMULA_LEADING_CHARS = ("=", "+", "-", "@", "\t", "\r")


def _neutralize_csv_formula(value):
    """Prefix a CSV cell with ``'`` if it would be read as a formula.

    ``username``/``full_name`` are JIT-provisioned from the external issuer's
    userinfo response, so a user can set them to a formula-injection payload
    (e.g. ``=HYPERLINK(...)``). Spreadsheet applications (Excel, Sheets,
    LibreOffice) treat a leading ``=``/``+``/``-``/``@``/tab/CR as a formula
    prefix; the OWASP-recommended mitigation is a leading single quote, which
    forces the cell to be read as literal text.
    """
    text = str(value) if value is not None else ""
    if text.startswith(_CSV_FORMULA_LEADING_CHARS):
        return "'" + text
    return text
...
def _export_users():
    ...
    for user in Session.query(User).all():
        csv_writer.writerow(
            [
                user.id_,
                _neutralize_csv_formula(user.email),
                _neutralize_csv_formula(user.username),
                _neutralize_csv_formula(user.full_name),
            ]
        )
    return csv_file_obj
...
def _import_users(users_csv_file):
    ...
    for row in csv_reader:
        if len(row) == 5:
            # Legacy exports were: id, email, access_token, username,
            # full_name. Tokens are intentionally not restored.
            id_, email, _legacy_token, username, full_name = row
        elif len(row) == 4:
            id_, email, username, full_name = row
        else:
            raise ValueError(
                "User CSV rows must contain either 4 columns or the legacy "
                "5 columns."
            )
        user = User(
            id_=id_,

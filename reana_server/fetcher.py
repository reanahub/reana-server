...
class WorkflowFetcherGit(WorkflowFetcherBase):
    ...
    @staticmethod
    def _remote_refs(repository: Repo) -> set[str]:
        """Get the remote heads and tags available in the repository."""
        with repository.git.custom_environment(GIT_TERMINAL_PROMPT="0"):
            remote_refs = repository.git.ls_remote(
                "--heads", "--tags", repository.remote().url
            )
        return {
            ref
            for line in remote_refs.splitlines()
            for ref in [line.split(None, 1)[1]]
            if not ref.endswith("^{}")
        }

    @classmethod
    def _git_ref_exists(cls, repository: Repo, git_ref: str) -> bool:
        ...
        if cls._looks_like_git_sha(git_ref):
            try:
                repository.commit(git_ref)
                return True
            except (BadName, ValueError):
                # GitPython raises ValueError when a valid-looking SHA is missing
    ...
    def _resolve_tree_path(
        ...
        remote_refs = cls._remote_refs(repository)
        for idx in range(len(path_parts), 0, -1):
            ...
            if (
                ...
                or cls._git_ref_exists(repository, git_ref)
            ):

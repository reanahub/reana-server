...
class KeycloakGroupBackend(GroupBackend):
    ...
    def _group_ref_from_representation(self, group):
        ...
        if not external_id:
            logging.warning("Skipping Keycloak group without immutable id: %r.", group)
            return None
    ...
    def _admin_get(self, path, params=None, ok_statuses=(200,)):
        ...
        try:
            ...
            raise GroupBackendError(f"Keycloak Admin API request failed: {error}")
        if response.status_code == 404:
            return None
        if response.status_code not in ok_statuses:
            raise GroupBackendError(
                f"Keycloak Admin API returned {response.status_code} " f"for {path}."
            )
        try:
    ...
    def fetch_memberships(self, user) -> List[GroupRef]:
        ...
        while True:
            ...
            if page is None:
                raise GroupBackendError(f"Keycloak user {user.idp_subject} not found.")
            for group in page:
